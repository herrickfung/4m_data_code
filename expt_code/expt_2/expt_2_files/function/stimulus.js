...
function createGabor(canvas){
  ...
  var trial_tilt = computeTrialTilt(triallist[triallist_no][trial][4], triallist[triallist_no][trial][5], triallist[triallist_no][trial][6])
  var numCyclesPer_stimSize = triallist[triallist_no][trial][3];   // for gabor stimulus, number of cycels per stimulus size
...
function createGaborEnd(data){
  ...
  data.stim_sf = triallist[triallist_no][trial][3];
  data.stim_tilt_diff = triallist[triallist_no][trial][4];
  data.stim_base_tilt = triallist[triallist_no][trial][5];  // tilt left or right array
  data.tilt_more_to_left = triallist[triallist_no][trial][6];
  data.stim_tilt = computeTrialTilt(triallist[triallist_no][trial][4], triallist[triallist_no][trial][5], triallist[triallist_no][trial][6]);
};
...
function createPerceptRespScreen(canvas){
  ...
  var ref_coor_start = getRefLineCoordinates(triallist[triallist_no][trial][5], stimSize, window_dims, true);
  var ref_coor_end = getRefLineCoordinates(triallist[triallist_no][trial][5], stimSize, window_dims, false);
  ctx.beginPath()
...
function createStartPracticeScreenD2(){
  var instruction;
  var trial_no = jsPsych.data.getLastTrialData().select('trial_no').values[0];
  var curr_triallist = jsPsych.data.getLastTrialData().select('curr_triallist').values[0];
  if (typeof curr_triallist === "undefined"){   // this will catch for the first trial
  ...
  switch (curr_triallist){
    case 0:
      ...
      break;
    case 1:
      instruction = start_prac_instruct_4;
      break;
  };
 return instruction;
};
...
function createEndBreakScreen(){
  ...
  var run_block_no_text =
    ...
    '<br><br> RUN: &nbsp ' + run_no + '/5 <br><br> BLOCK: ' + block_no + '/5 ';

  return run_block_no_text;
};

function createConfirmInfoScreen(){
  optimal_tilt = getOptimal();
  subject_id = getSubjNo();

  var no_record_text =
    '<p style="font-size:32px;color:#FF0000;text-align:left">' +
    'SOMETHING WENT WRONG!!! </p>' +
    '<p style="font-size:20px;color:#000000;text-align:left">' +
    'Your Prolific ID was recorded as ' + subject_id +
    "<br><br> It appears that you have not completed Day 1 of the Experiment or <br> you have listed a different Prolific ID as before." +
    "<br><br> Reload the page and enter a new Prolific ID." +
    "<br><br> Let us know if the problem persists." +
    '</p>';
  var have_record_text =
    '<p style="font-size:20px;color:#000000;text-align:left">' +
    'Your Prolific ID was recorded as ' + subject_id +
    '<br><br> Welcome back to Day 2 of the experiment!' +
    '<br><br> Please proceed only if you have received an invitation for this day. <br> Otherwise, you will not be reimbursed for completing this session.' +
    '<br><br> If you did receive an invitation, press "+" to continue with the experiment.' +
    '</p>';

if (isNaN(optimal_tilt)){
    confirm_text = no_record_text;
  } else {
    confirm_text = have_record_text;
  };
  return confirm_text;
};
